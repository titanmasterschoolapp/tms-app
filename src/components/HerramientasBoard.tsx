...
export default function HerramientasBoard({ 
  ...
  const handleApproveReply = async (topicId: string, replyId: string) => {
    try {
      await DataAPI.approveTopicReply(topicId, replyId, 'tool');
      onRefresh();
    } catch (e: any) {
  ...
  const handleRejectReply = async (topicId: string, replyId: string) => {
    try {
      await DataAPI.rejectTopicReply(topicId, replyId, 'tool');
      onRefresh();
    } catch (e: any) {
                    ...
                    <div className="bg-[#050506] border border-white/[0.03] rounded-2xl p-3 flex-grow max-h-[160px] overflow-y-auto space-y-2 pr-1 select-text">
                      {(() => {
                        ...
                        if (visibleReplies.length === 0) {
                          return <p className="text-center text-[10px] text-zinc-650 italic py-6 font-mono uppercase tracking-wider">No hay comentarios en este debate de soporte</p>;
                        }
                        ...
                        return visibleReplies.map((rep) => {
                          ...
                          return (
                            <div id={`tool-rep-${rep.id}`} key={rep.id} className={`p-2 border border-white/[0.03] bg-zinc-950/35 rounded-xl space-y-1 ${isPending ? 'border-amber-500/20 bg-amber-500/[0.01]' : ''}`}>
